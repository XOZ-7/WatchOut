...
const recentActivity = [
  ...
  { id: 4, content: 'Lab report verification for blood test', flag: 'Verified', confidence: 99, time: '12 ago' },
];
...
export default function Dashboard({ setActiveTab }: DashboardProps) {
  const [activeSubTab, setActiveSubTab] = React.useState<'system' | 'analytics'>('system');
  ...
  return (
    <div className="p-8 space-y-8 bg-[#0a0a0a] min-h-full text-white overflow-y-auto scrollbar-hide">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight font-sans">Medical Guard Dashboard</h1>
          <p className="text-white/40 text-sm mt-1">Real-time monitoring of medical misinformation and system health.</p>
        </div>
        <div className="flex items-center gap-2 bg-white/5 p-1 rounded-2xl border border-white/10">
          <button 
            ...
            className={cn(
              ...
              activeSubTab === 'system' ? "bg-emerald-500 text-black shadow-lg shadow-emerald-500/20" : "text-white/40 hover:text-white"
            )}
          >
            System
          </button>
          <button 
            ...
            className={cn(
              "px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all",
              activeSubTab === 'analytics' ? "bg-emerald-500 text-black shadow-lg shadow-emerald-500/20" : "text-white/40 hover:text-white"
            )}
          >
            Analytics
          </button>
